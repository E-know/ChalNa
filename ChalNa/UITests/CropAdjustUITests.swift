...
///   단위로 완전히 동일했다(2회 모두 재현). **이 관찰은 완전히 설명된다.** dev fixture 4개
///   (`Modules/PhotosService/Resources/DevFixtures/*.mp4`)는 전부 `mdls`/
///   `AVAssetTrack.naturalSize` 로 교차 확인한 **360×640 세로(9:16)** 라 1080×1920 캔버스와
///   종횡비가 정확히 일치한다 — `fillScale = max(1080/360, 1920/640) = max(3.0, 3.0) = 3.0`
///   이라 스케일된 전경(1080×1920)이 캔버스와 정확히 같고, 옛 `maxOffsetFraction`(scale 1
///   기준)은 **양축 모두 (0, 0)** 이었다. 즉 어느 방향으로 드래그해도 결과가 달라질 수 없었다.
///   또한 옛 `ClipAdjustView` 는 핀치를 `min(max(scale, 1.0), 4.0)` 로 클램프해
///   `app.pinch(withScale: 0.5)` 가 scale 1.0 으로 커밋됐다. 즉 이 체크포인트들은 **당시
///   코드의 명세된 동작 그 자체**였다 — "XCUITest 합성 제스처가 뷰에 관측 가능한 변화를 못
///   만든다"는 결론은 이 관찰로 뒷받침되지 않으며, 진짜 원인이 밝혀졌으니 이제 그런 결론을 낼
///   필요도 없다. 다음 사람이 같은 조사를 반복하거나 그 잘못된 결론으로 다시 가지 않도록
///   남겨둔다.
///
