...
- Update `tasks/todo.md` only for long-running, multi-step, or architecture-level work.
- For small UI fixes, narrow bug fixes, or single-file changes, skip task-log updates unless the user explicitly asks for them.
- Keep final responses as the source of truth for small-change summaries.
...
├─ Tokens/       ChalNaColor · Typography · Spacing · Radius · Shadow
...
### Spacing — `ChalNaSpacing` (4pt base)
...
- `.padding(17)` 같은 비-4pt 값 금지 — `ChalNaSpacing.md` 사용
