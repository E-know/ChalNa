...
- **UI 테스트**는 별도로 `ChalNaUITests` 타겟(`ChalNa` 앱 타겟에 의존, `-scheme ChalNa`로 실행)에 네 파일이 있다 — `CropAdjustUITests` · `HomeLibraryCoverUITests`(export → 라이브러리 저장 → 홈 노출의 유일한 엔드투엔드 검증이자 표지 스크린샷 소스) · `LabelEditorUITests`(2스텝 플로우 컨트롤·키보드 도달성 1케이스 + `testLabelEditor_DynamicTypeAccessibility5_CapsAtAccessibility1` — "크기" 캡션 렌더 높이를 실측하는 Dynamic Type 상한 가드 1케이스) · `TimelineLayoutUITests`. 여기가 **XCTest 의 유일한 예외 대상**이다(아래 "기술 스택" 참고) — 다만 devMock 픽스처를 쓰는 이유는 사진 권한 없이 화면에 도달하기 위해서일 뿐이고, **시각 검증 채널이 아니다**. `CropAdjustUITests`는 도달성·무크래시 스모크 테스트로만 취급하지만, `HomeLibraryCoverUITests`의 엔드투엔드 단언·`LabelEditorUITests`의 Dynamic Type 케이스·`TimelineLayoutUITests`의 오프스크린 기하 가드(하단 툴바 4개·재생 버튼에 `screen.contains(button.frame)`)는 실제로 값을 재는 가드다 — 근거와 실제 기하·라벨 가드의 위치는 아래 "검증 방법" 절 참고.
