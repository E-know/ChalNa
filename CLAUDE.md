...
- **UI 테스트**는 별도로 `ChalNaUITests` 타겟(`ChalNa` 앱 타겟에 의존, `-scheme ChalNa`로 실행)에 있다 — `CropAdjustUITests` · `LabelEditorUITests`(2스텝 플로우 컨트롤·키보드 도달성 1케이스 + Dynamic Type 상한 1케이스) · `TimelineLayoutUITests`. 여기가 **XCTest 의 유일한 예외 대상**이다(아래 "기술 스택" 참고) — 다만 devMock 픽스처를 쓰는 이유는 사진 권한 없이 화면에 도달하기 위해서일 뿐이고, **시각 검증 채널이 아니다**. 셋 모두를 도달성·무크래시 스모크 테스트로만 취급하는 근거와 실제 기하·라벨 가드의 위치는 아래 "검증 방법" 절 참고.
