...
| 4 | 가치③ | "그날의 시간까지 함께 기록" — "시간·날짜 라벨은 자동, 자막은 원하는 대로 / 필요 없으면 언제든 끌 수 있어요" | 라벨 오버레이 목업(시간 09:30 + 날짜) + **자막 박스("제주 바다", LabelEditor 스타일)** + **시간/날짜 라벨 토글**(설정 화면과 동일한 장치로 "선택 사항"임을 시각화) |
