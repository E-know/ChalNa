...
```
첫 실행:   Splash → 가치① → 맞춤 질문 → 가치② → 가치③(시간·날짜 라벨) → 가치④(자막) → 권한 안내 → 페이월(하드) → 체험 시작 → Home
...
## 3. 화면 구성 (7화면, Figma `1.2.0` 페이지 컨벤션: 393×852 / DDS 토큰 / 한글 Noto Sans KR)
...
| 4 | 가치③ | "그날의 시간까지 함께 기록" — "영상 위에 시간과 날짜가 자동으로 새겨져요 / 필요 없으면 언제든 끌 수 있어요" | 라벨 오버레이 목업(시간 09:30 + 날짜) + **시간/날짜 라벨 토글**(설정 화면과 동일한 장치로 "선택 사항"임을 시각화) |
| 5 | **가치④ · 자막 (별도 페이지)** | "하고 싶은 말은 자막으로" — "원하는 클립에 자막을 얹고 / 위치와 크기도 자유롭게 바꿔요" | 자막 박스("제주 바다", LabelEditor 박스 스타일) + **점선 선택 프레임**(편집 중 상태) + **크기 슬라이더** 미니어처 |
| 6 | 권한 안내 | "추억을 불러올게요" — 선택한 사진에만 접근 | 권한 프라이밍 + 민감 요청 완충 카피 (Noom 패턴) |
| 7 | 페이월 | "ChalNa Pro — 3일 동안 모든 기능이 무료예요" | 아래 상세 |

- 온보딩(1~6) 공통: 페이지 dots 6개 + `chalNa(.filled, .xl, fillWidth)` CTA, 스와이프 전환.
